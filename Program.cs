...
namespace TP3_Trading_ROUVIER_Hugo
{
    public class MovieCollection
    {
        ...
        static void Main(string[] args)
        {
            ...
            var all =
                (from m in context.Movies
                 select m).Count();
            Console.WriteLine("Their are {0} movie in the collection\n", all);
            ...
            var e =
                (from m in context.Movies
                 ...
                 select m).Count();
            Console.WriteLine("Their are {0} movie in the collection with the letter 'e' at least once in their title \n", e);

            Console.WriteLine("\nQuestion 4 : \n");
            var f =
                (from m in context.Movies
                 select m.Title.Count(f => f == 'f')).Sum();
            Console.WriteLine("Their are {0} time the letter 'f' in all titles \n", f);
            ...
            var higherBudget =
                (from m in context.Movies
                 ...
                 select m).First();
            Console.WriteLine("{0}, is the movie with the higher budget (${1})\n", higherBudget.Title, higherBudget.Budget);
            ...
            var lowestBox =
                (from m in context.Movies
                 ...
                 select m).First();
            Console.WriteLine("{0}, is the movie with the lowest box office (${1}), we do not take into account data that seems wrong (box office < $1000)\n", lowestBox.Title, lowestBox.BoxOffice);
            ...
            int i = 0;
            Console.WriteLine("The 11 first movie order by ascending title are : \n");
            foreach (string m in eleven)
            {
            ...
            Console.WriteLine("The mean of all Budget / Box Office of every movie ever is {0} (if we do not count null data and wrong data)", meanBB);

            Console.WriteLine("\nQuestion 12 : \n");
            var harder =
                from m in (from m in context.Movies
                           group m.Title by m.Title.Length into newgroup
                           select newgroup)
                orderby m.Key ascending
                select m;

            Console.WriteLine("Number of films for each number of characters in the title screen: \n");
            foreach (var m in harder)
            {
                Console.WriteLine("{0} char => {1} film \n", m.Key, m.Count());
            }

            Console.WriteLine("\nQuestion 12 : \n");
            var date =
                (from a in (from m in context.Movies
                            where (m.BoxOffice != null && m.Budget != null)
                            group (m.Budget/m.BoxOffice) by m.ReleaseDate.Year into newgroup
                            select newgroup)
                orderby a.Key ascending
                select a);

            Console.WriteLine("Mean of all Budget / Box Office of every movie grouped by yearly release date: \n");
    ...
    public class Exercice2
    {
        ...
        public void skyofstar()
        {
            ...
            Thread emptySpace = new Thread(new ThreadStart(() => ThreadProc(' ', 50))); 
            emptySpace.Start();
